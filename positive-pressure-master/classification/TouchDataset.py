...
class TouchDataset(data.Dataset):
    ...
    def refresh(self):
        ...
        indices0 = np.argwhere(self.valid0).flatten()#所有标记为一次采集的第一帧的数据的索引
        self.indices = np.zeros((len(indices0), self.sequenceLength), int)
        for i in range(len(indices0)):#遍历所有可用的数据
            ind0 = indices0[i]#对应编号
            mask = self.validN.copy()
            if 'recordingId' in self.meta:
                mask = np.logical_and(mask, self.meta['recordingId'] == self.meta['recordingId'][ind0])#所有和ind0相同Id的数据
            mask[ind0] = False
            candidates = np.argwhere(mask).flatten()
            np.random.shuffle(candidates)
            self.indices[i,0] = ind0 #所有的sequence的第一帧
            self.indices[i,1:] = candidates[:(self.sequenceLength - 1)]#数据里随机选其他[sequenceLength-1]个数据,其实就是针对每个label做shuffle
    ...
    def transformPressure(self, pressure):
        pressure = np.clip((pressure.astype(np.float32) - 500) / (650 - 500), 0.0, 1.0)#将将数组中的元素限制在a_min, a_max之间
        return pressure
